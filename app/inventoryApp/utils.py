        ...
        if user.marketplace_name == "Ebay":
            ...
            for item in all_ebay_items:
                try:
                   ...
                   # Get list of vendors registered by the user
                    ...
                    print(f"Ebay Product processing in first try block failed with error with item id {item.get('ebay_item_id')}: {e}")
            ...
            for item in all_woocommercer_items:
                try:
                    ...
                    print(f" Woocommerce Product processing in first try block failed with error with item id {item.get('id')}: {e}")
                    # If item does not exist, insert new item
                    try:
