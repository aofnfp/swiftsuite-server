                ...
                try:
                    ...
                    if db_item:
                        try:
                            ...
                            inentory, created = InventoryModel.objects.update_or_create(Q(market_item_id=item.get("ebay_item_id")) | Q(sku=item.get("ebay_sku")), defaults={"map_status": True, "product_id": item_product.id, "vendor_name": db_item.vendor.name, "market_item_url": item.get("market_item_url")})
                    ...
                    try:
                        ...
                        "country": product_details.get("itemLocation")["country"], "price": product_details.get("price")["value"], "thumbnailImage": product_details.get("additionalImages"), "vendor_name": "Not Found", "map_status": False, "market_name": "Ebay", "fixed_percentage_markup":user.fixed_percentage_markup, "fixed_markup":user.fixed_markup, "profit_margin":user.profit_margin, "min_profit_mergin":user.min_profit_mergin, "charity_id":user.charity_id, "enable_charity":user.enable_charity, "market_item_url": item.get("market_item_url")})
                ...
                try:
                    ...
                    if db_item:
                        try:
                            ...
                            # Item exists, check if we need to update price or quantity
                            inentory, created = InventoryModel.objects.update_or_create(Q(market_item_id=item.get("id")) | Q(sku=item.get("sku")), defaults={"map_status": True, "product_id": item_product.id, "vendor_name": db_item.vendor.name, "market_item_url": item.get("market_item_url")})
                            # Update the VendorUpdate table to set listed_market to true
                            db_item.active = True
                            db_item.save()
                    ...
                    # If item does not exist, insert new item
                    try:
                        item_to_save, created = InventoryModel.objects.update_or_create(user_id=user.user_id, market_item_id=item.get("id"), defaults=dict(title=item.get("name") or "NA", description=json.dumps(item.get("description")) or "NA", category_id=item.get("categories")[0]["id"] or 0, sku=item.get("sku") or 0, start_price=item.get("price") or 0, picture_detail=item.get("images")[0].get("src") or "NA", quantity=item.get("stock_quantity") or 0, return_profileID="Null", return_profileName="Null", payment_profileID="Null", payment_profileName="Null", shipping_profileID="Null", shipping_profileName="Null", categoryMappingAllowed="", item_specific_fields="Null", market_logos="Null", date_created=item.get("date_created").split("T")[0] or "NA", active=True, category=item.get("categories")[0].get("name") or "NA", price=item.get("price") or "NA", thumbnailImage="Null", vendor_name="Not Found", enable_charity=True, woo_category_name=item.get("categories")[0].get("name") or "NA", market_name="Woocommerce", map_status=False, fixed_percentage_markup=user.fixed_percentage_markup, fixed_markup=user.fixed_markup, profit_margin=user.profit_margin, min_profit_mergin=user.min_profit_mergin, market_item_url=item.get("permalink") or "NA"))
