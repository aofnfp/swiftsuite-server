            ...
            try:
                ...
                for vendor_db in vendor_list:
                    try:
                        ...
                        item_listing, created = Generalproducttable.objects.update_or_create(user_id=user.user_id, sku=db_item.get("sku"), defaults=dict(active=True, upc=item_exists.upc, map=db_item.product.map, mpn=item_exists.mpn, enrollment_id=db_item.get("enrollment_id"), product_id=db_item.get("product_id"), quantity=db_item.get("quantity"), total_price=db_item.get("total_price"), vendor_id=db_item.get("vendor_id"), vendor_name=db_item.vendor.name))
                        break                    
                ...
                if db_item:
                    ...
                    selling_price = calculated_selling_price(enroll_id=db_item.get("enrollment_id"), market_id=user._id, start_price=db_item.get("total_price"), userid=user.user_id, map=db_item.product.map)
                    if selling_price == None:
                        continue
                    # Item exists, check if we need to update price or quantity
                    InventoryModel.objects.filter(Q(ebay_item_id=item.get("ebay_item_id")) | Q(sku=item.get("ebay_sku"))).update(start_price=selling_price, quantity=db_item.get("quantity"), map_status=True, product_id=db_item.product.id, ebay_item_id=item.get("ebay_item_id"), vendor_name=db_item.vendor.name)
