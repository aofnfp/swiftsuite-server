                ...
                try:
                    ...
                    if db_item:
                        try:
                            ...
                            item_product, created = Generalproducttable.objects.update_or_create((Q(user_id=user.user_id) & Q(sku=db_item.sku)) & (Q(mpn=item_exists.mpn) | Q(upc=item_exists.upc)), defaults=dict(active=True, total_product_cost=total_product_cost, map=db_item.product.map, enrollment_id=db_item.enrollment_id, product_id=db_item.product_id, quantity=db_item.quantity, price=db_item.total_price, vendor_name=db_item.vendor.name))
