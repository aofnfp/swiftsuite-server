        ...
        if user.marketplace_name == "Ebay":
            ...
            for item in all_woocommercer_items:
                try:
                    ...
                    # If item does not exist, insert new item
                    try:
                        item_to_save, created = InventoryModel.objects.update_or_create(user_id=user.user_id, market_item_id=item.get("id"), defaults=dict(title=item.get("name"), description=json.dumps(item.get("description")), category_id=item.get("categories")[0]["id"] or 0, sku=item.get("sku"), start_price=item.get("price"), picture_detail=item.get("images")[0].get("src") or None, quantity=item.get("stock_quantity"), return_profileID="Null", return_profileName="Null", payment_profileID="Null", payment_profileName="Null", shipping_profileID="Null", shipping_profileName="Null", categoryMappingAllowed="", item_specific_fields="Null", market_logos="Null", market_item_id=item.get("id"), user_id=user.user_id, date_created=item.get("date_created"), active=True, category=item.get("categories")[0].get("name") or None, price=item.get("price"), thumbnailImage="Null", vendor_name="Not Found", enable_charity=True, woo_category_name=item.get("categories")[0].get("name") or None, market_name="Woocommerce", map_status=False))
